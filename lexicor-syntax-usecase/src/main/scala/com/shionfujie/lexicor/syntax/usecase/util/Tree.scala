...
private[syntax] object Tree {
  ...
  private object factory {
    ...
    def apply[K, D](pair: (K, D), pairs: (K, D)*): Tree[K, D] = {
      ...
      pairs0.foldLeft(factory(key0, data0)) { case (tree, (key1, data1)) => factory(key1, data1, tree) }
    }

    private def apply[K, D](key: K, value: D, tree: Tree[K, D] = empty[K, D]): Tree[K, D] = new Tree(Set((Entry(key, value), tree)))
